package bunq

import (
	"fmt"
	"net"
	"net/http"
	"net/http/httptest"
	"reflect"
	"strings"
	"testing"
	"time"
)

func TestCreateDeviceServer(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprintln(w, `{"Response":[{"Id":{"id":83}}]}`)
	}))
	defer ts.Close()

	client := NewClient()
	client.BaseURL = ts.URL

	privKey := `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`
	r := strings.NewReader(privKey)
	if err := client.SetPrivateKey(r); err != nil {
		t.Fatal(err)
	}

	got, err := client.CreateDeviceServer("Foobar", []net.IP{})
	if err != nil {
		t.Fatal(err)
	}

	exp := &DeviceServer{
		ID: 83,
	}

	if eq := reflect.DeepEqual(exp, got); !eq {
		t.Errorf("Expected: `%#v`, got: `%#v`", exp, got)
	}
}

func TestGetDeviceServer(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprintln(w, `{"Response":[{"DeviceServer":{"id":42,"created":"2015-06-13 23:19:16.215235","updated":"2015-06-30 09:12:31.981573","description":"Mainframe23 in Amsterdam","ip":"255.255.255.255","status":"ACTIVE"}}]}`)
	}))
	defer ts.Close()

	client := NewClient()
	client.BaseURL = ts.URL

	privKey := `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`
	r := strings.NewReader(privKey)
	if err := client.SetPrivateKey(r); err != nil {
		t.Fatal(err)
	}

	got, err := client.GetDeviceServer(42)
	if err != nil {
		t.Fatal(err)
	}

	exp := &DeviceServer{
		ID:          42,
		CreatedAt:   time.Unix(1434237556, 215235000).UTC(),
		UpdatedAt:   time.Unix(1435655551, 981573000).UTC(),
		Description: "Mainframe23 in Amsterdam",
		IP:          net.IPv4bcast,
		Status:      "ACTIVE",
	}

	if eq := reflect.DeepEqual(exp, got); !eq {
		t.Errorf("Expected: `%#v`, got: `%#v`", exp, got)
	}
}

func TestListDeviceServers(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprintln(w, `{"Response":[{"DeviceServer":{"id":42,"created":"2015-06-13 23:19:16.215235","updated":"2015-06-30 09:12:31.981573","description":"Mainframe23 in Amsterdam","ip":"255.255.255.255","status":"ACTIVE"}},{"DeviceServer":{"id":43,"created":"2015-06-13 23:19:16.215235","updated":"2015-06-30 09:12:31.981573","description":"Mainframe24 in Amsterdam","ip":"1.2.3.4","status":"ACTIVE"}}]}`)
	}))
	defer ts.Close()

	client := NewClient()
	client.BaseURL = ts.URL

	privKey := `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`
	r := strings.NewReader(privKey)
	if err := client.SetPrivateKey(r); err != nil {
		t.Fatal(err)
	}

	got, err := client.ListDeviceServers()
	if err != nil {
		t.Fatal(err)
	}

	exp := []*DeviceServer{
		&DeviceServer{
			ID:          42,
			CreatedAt:   time.Unix(1434237556, 215235000).UTC(),
			UpdatedAt:   time.Unix(1435655551, 981573000).UTC(),
			Description: "Mainframe23 in Amsterdam",
			IP:          net.IPv4bcast,
			Status:      "ACTIVE",
		},
		&DeviceServer{
			ID:          43,
			CreatedAt:   time.Unix(1434237556, 215235000).UTC(),
			UpdatedAt:   time.Unix(1435655551, 981573000).UTC(),
			Description: "Mainframe24 in Amsterdam",
			IP:          net.ParseIP("1.2.3.4"),
			Status:      "ACTIVE",
		},
	}

	if eq := reflect.DeepEqual(exp, got); !eq {
		t.Errorf("Expected: `%#v`, got: `%#v`", exp, got)
	}
}
